---
slug: corazon-new-farm
title: Six Ensuites and a Climbing Wall
suburb: New Farm
state: QLD
tags:
  - Luxury Excess
  - Statement Piece
era: Queenslander-to-contemporary total rebuild
beds: 6
baths: 8.5
cars: 6
quirk: 5
verdict: Made the list for a staircase that spirals through three storeys like a piece of sculpture, a bedroom with its own indoor climbing wall, and a “wellness retreat” tucked inside the garage.
youtubeId: GtWTnvXZSs4
heroImage: https://images.theplist.com.au/corazon-04-staircase.jpg
listingUrl: https://www.realestate.com.au/property-house-qld-new+farm-151221944
photos:
  - src: https://images.theplist.com.au/corazon-01-street.jpg
    caption: The street view — Queenslander bones, full renovation
  - src: https://images.theplist.com.au/corazon-02-rear-twilight.jpg
    caption: The rear at twilight, hanging gardens and all
  - src: https://images.theplist.com.au/corazon-03-pool.jpg
    caption: The pool, signed like the house
  - src: https://images.theplist.com.au/corazon-04-staircase.jpg
    caption: The staircase that, by all accounts, goes for days
  - src: https://images.theplist.com.au/corazon-05-great-room.jpg
    caption: The great room, marble island included
  - src: https://images.theplist.com.au/corazon-06-kitchen.jpg
    caption: The kitchen — Miele, Gaggenau, more marble
  - src: https://images.theplist.com.au/corazon-07-primary-suite.jpg
    caption: The primary suite, glass-encased and garden-immersed
  - src: https://images.theplist.com.au/corazon-08-ensuite.jpg
    caption: One of six ensuites
  - src: https://images.theplist.com.au/corazon-09-rooftop.jpg
    caption: The rooftop bar, city skyline included
date: '2026-06-21'
order: 28
---

Every now and then The P List finds a house that didn't need rescuing from beige — it needed three firms (architecture, construction, and the property group behind it), a rebuild down to the studs, and apparently no upper limit on the marble budget. ‘Corazón’ started life as a classic Queenslander. It does not look like one anymore, beyond a nod to the old roofline kept on as a hint of where it came from.

The centrepiece is a staircase that spirals through all three levels like a single continuous piece of sculpture — the kind of thing you photograph from four angles before remembering you were actually just trying to get to the kitchen.

Said kitchen runs Miele and Gaggenau behind a slab of marble big enough to need its own postcode, backed by a butler's pantry, a wine bar, and two further wet bars for redundancy. The numbers keep going from there: six bedrooms, eight and a half bathrooms, a dedicated children's wing with its own indoor climbing wall and a suspended net cubby, and a garage that includes — alongside, presumably, the actual cars — a wellness retreat. The pool has its name lettered straight into the tile, in case the rest of the house left any doubt about the theme.

It's going to auction with no price guide, which feels about right for a house that was never going to fit neatly into a column anyway. Two hundred metres from New Farm Park, for anyone who finds themselves in the fortunate position of inspecting.
